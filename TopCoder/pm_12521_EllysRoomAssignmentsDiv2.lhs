...
Problem Statement: EllysRoomAssignmentsDiv2
...
1. Let the number of registrants be N.
2. The number of rooms is set to be R = N / 20, if N is divisible by 20, or
R = N / 20 + 1 otherwise. Here the '/' operator represents integer division,
i.e., the remainder is being ignored.
3. The registrants are sorted by rating in decreasing order.
4. The first R registrants are randomly assigned to rooms such that no two of
them are in the same room. Then the same is done with the next R registrants and
so on, until there are no more unassigned registrants.
Note that it is possible for the last group to have less than R registrants, but
they are still randomly assigned into rooms in such a way that no two of them
are in the same room.
You are given a String[] ratings. Concatenate its elements to obtain one long
string. It will represent a space-separated list of the contestants' ratings.
For simplicity, no two ratings in the list will be equal. Elly's rating is the
first one in the list.

Return the probability that she will be assigned to the room to which the person
with the highest rating is assigned.
...
There are 43 registrants, one of them is Elly. Her raiting is 724.
The contestants will be distributed into (43/20) + 1 = 3 rooms. One of the rooms
...
> parseRatings :: String -> [Int]
> parseRatings str = map read $ words str

> parseRatings' xs = foldl (\r s -> r ++ parseRatings s) [] xs

> orderRatings :: [Int] -> [Int]
> orderRatings = sortBy (\a b -> b `compare` a)

> example1 = [ "724 42 13 17 1199 577 1001 1101 483 845 196 1163 3"
...
> example2 = ["42 911 666 17 13 1 1155 1094 815 5 1000 540"]
>
> example3 = ["1168 196 440 643 227 1194 1149 372 878 23 767 296 ",
...
> example4 = ["470 840 410 1168 637 464 498 1118 276 1013 989 874",
...
>       eRating:_ = ratings
>       rooms = numRooms $ length ratings
>       ordRatings = orderRatings ratings
>       computeProbability = case ellyPosition eRating ordRatings of
>                             Nothing -> strResult 0
>                             Just pos -> strResult pos
>       strResult pos = ("Rooms: " ++ show rooms,
>                        "Position: " ++ show pos,
>                        "Rem: " ++ show (pos `rem` rooms),
>                        fromIntegral (pos `rem` rooms) / fromIntegral rooms)
>   in computeProbability

> answer = mapM_ (print . ellyProbability) examples
>   where
>     examples = [ example1
>                , example2
>                , example3
...
> main :: IO ()
> main = do
>   putStrLn "*** Solution to EllysRoomAssignmentsDiv2 ***"
>   answer
